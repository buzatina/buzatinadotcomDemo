...
router.get('/near', function(req, res){
	...
	console.log('Was the method called?');

	if (req.user.picUrl) {
 
		    User.find({}, function(err, nearEntrepreneurs){

					if(err){
						console.log(err)
					}

		        	var objQ = {question: nearEntrepreneurs.reverse()};

		        	console.log(objQ);

		            res.render('near', objQ);
		...
		res.render('near', {buzaLocalErr: 'The was an error sending the things boss'});
